import { X, Plus, Minus, Trash2, ShoppingBag, ShoppingCartIcon, MapPin, User, Phone, Mail, Check } from 'lucide-react';
...
export function CartDrawer() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      goToPayment(); // Vai direto para pagamento
    }
  };
  ...
  const handleCheckout = () => {
    if (customerInfo.nome && customerInfo.endereco && customerInfo.telefone) {
      // Se já tem endereço salvo, vai direto para pagamento
      goToPayment();
    } else {
              ...
              {customerInfo.nome && (
                <div className="mb-4 bg-green-50 border border-green-200 rounded-lg p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="font-medium text-green-800">Endereço salvo:</p>
                      <p className="text-sm text-green-700">{customerInfo.nome}</p>
                      <p className="text-sm text-green-600 truncate">{customerInfo.endereco}</p>
                      <button
                        onClick={() => {
              ...
              {cart.length > 0 && (
                <div className="border-t p-4 space-y-4">
                    ...
                    {customerInfo.nome && (
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mt-2">
                        <p className="text-sm text-blue-800">
                          <span className="font-medium">Entrega para:</span> {customerInfo.nome}
                        </p>
                        <p className="text-xs text-blue-600 truncate">{customerInfo.endereco}</p>
                      </div>
                    )}
                  </div>
                  ...
                  <Button
                    ...
                    size="lg"
                  >
                    {customerInfo.nome ? 'Continuar para Pagamento' : 'Finalizar Compra'}
                  </Button>
